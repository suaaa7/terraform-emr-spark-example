...
resource "aws_s3_bucket_object" "log4j_spark" {
  ...
  etag   = filemd5("${path.module}/files/log4j_spark.properties")
}
...
resource "aws_s3_bucket_object" "log4j_zeppelin" {
  ...
  etag   = filemd5("${path.module}/files/log4j_zeppelin.properties")
}
...
resource "aws_s3_bucket_object" "logrotate" {
  ...
  etag   = filemd5("${path.module}/files/logrotate")
}
...
resource "aws_s3_bucket_object" "logrotate_script" {
  ...
  etag   = filemd5("${path.module}/files/logrotate.sh")
}
...
resource "aws_s3_bucket_object" "syslog" {
  ...
  etag   = filemd5("${path.module}/files/syslog.conf")
}
